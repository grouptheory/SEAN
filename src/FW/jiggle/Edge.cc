...
// future upgrades.
#ifndef LINT
static char const _Edge_cc_rcsid_[] =
"$Id: Edge.cc,v 1.1 1999/01/21 14:54:39 mountcas Exp $";
#endif
...
Edge::Edge(Node * f, Node * t) 
  : from(f), to(t), isDirected(false), reversed(false) { }

Edge::Edge(Node * f, Node * t, bool dir)
  : from(f), to(t), isDirected(dir), reversed(false) { }

Edge::~Edge() { }

char * Edge::toString(void) const
{
  char buf[256];
  sprintf(buf, "%s %s %s", from->toString(), 
	  (isDirected ? "->" : "-"), to->toString());
  return strdup(buf);
}
