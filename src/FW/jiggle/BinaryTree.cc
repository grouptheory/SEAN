...
// future upgrades.
#ifndef LINT
static char const _BinaryTree_cc_rcsid_[] =
"$Id: BinaryTree.cc,v 1.1 1999/01/21 14:54:39 mountcas Exp $";
#endif
...
BinaryTree::BinaryTree(int h, bool dir) 
  : Graph()
{ 
  int twoToTheHMinusOne = (int)pow(2, h) - 1, i = 0;
  Node ** N = new Node * [twoToTheHMinusOne];
  for (i = 0; i < twoToTheHMinusOne; i++)
    insertNode(N[i] = new Node ());
  for (i = 1; i < twoToTheHMinusOne; i++)
    insertEdge(new Edge (N [(i - 1) / 2], N [i], dir));
  delete [] N;
}
