...
// -*- C++ -*-
// File: NeverAccept.cc
// Version: $Id: NeverAcceptTest.cc,v 1.4 1998/07/30 20:35:41 talmage Exp $
//
// Purpose: main() for the instantiation of NeverAccept.  See
// NeverAccept.cc
...
int main(int argc, char **argv)
{
  ...
  DiagLevel("NeverAccept", DIAG_INFO);
  ...
  // "OneService.output".  This lets us run live with GT if we
  // want to.
  //
    if (argc > 5)
      VisPipe(argv[4]);
    else 
    VisPipe("NeverAccept.output");

    NeverAccept *smanager;
    ...
    if (port > 0) {
      ...
      smanager = new NeverAccept("NeverAccept", host_nic, lifetime);
