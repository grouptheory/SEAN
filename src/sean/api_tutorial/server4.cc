...
"$Id: server4.cc,v 1.4 1998/08/20 20:25:08 talmage Exp $";
#endif
...
// will echo any data that arrives on an active connection.  This
// server notices when a connection becomes inactive.
//
// NEW: When an incoming connection arrives, the server deregisters
// its service.  When an connection becomes inactive, the server
// registers its service.  In this way only one call is ever active.
// **************************************************************************
...
private:
  ...
  void Call_Inactive(ATM_Call& call) {
    ...
    // Create a new service just like the old one we Deregister()ed
    // when in Incoming_Call().
    ATM_Service * serv = new ATM_Service(*_interface);
  ...
  // Service_Inactive will happen when 'service' is no longer ready to
  // receive calls.  
  void Service_Inactive(ATM_Service& service) {
    cout << "The service (id=" << service.Get_Service_ID() << ") is now inactive\n";
  ...
  void Incoming_Call(ATM_Service& service) {
    ...
    // Remove the service so that we only accept one call to it at a
    // time
    cout << "Requesting service (id=" << service.Get_Service_ID() << ") to be deregistered.\n";
