...
"$Id: client6.cc,v 1.3 1998/09/28 12:31:44 bilal Exp $";
#endif
...
private:
  ...
  void PeriodicCallback(int code) {
    ...
    case 3:		// Drop the second leaf
      CancelPeriodicCallback(code);

      cout << "Dropping Call (id=" << _outcall->Get_Call_ID() << ") ";
      cout << " leaf(id=" << _second_leaf->Get_Leaf_ID() << ")" << endl;

      // First, give the reason for dropping the leaf
      err = _second_leaf->Set_cause(ie_cause::normal_call_clearing);
      assert( err == ATM_Attributes::success );

      // Second, drop the leaf
      err = _second_leaf->Drop();
      assert( err == ATM_Attributes::success );
