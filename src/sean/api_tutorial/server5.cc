...
#ifndef LINT
static char const _server5_cc_rcsid_[] =
"$Id: server5.cc,v 1.2 1998/08/20 20:23:37 talmage Exp $";
#endif
#include <common/cprototypes.h>

// **************************************************************************
//
// This application is informed whenever ANY incoming connection comes in,
// and upon receiving said notification, it accepts the call.  These
// calls are never torn down, and the application assumes a best case
// scenario where all of its requests are granted.
//
...
#include <FW/basics/Conduit.h> // VisPipe()
class server_controller : public Controller {
public:
  server_controller(void)             { _total_bytes=0; _pdus=0; _data_callback=0; }
...
private:
  ...
  void Call_Inactive(ATM_Call& call) {
    ...
    int pdu_length;
    int bytes_read;
    cout << "Recieved " << _total_bytes << " bytes in "
	 << _pdus << " pdus in " << _data_callback << " call backs\n";
     _total_bytes=0; _pdus=0; _data_callback=0;
    call.TearDown();
  ...
  void Incoming_Data(ATM_Call &call) {
    ...
    if ((pdu_length = call.Next_PDU_Length()) > 0) {
      //      buffer = new u_char[pdu_length];
      bytes_read = call.RecvData(_buffer, pdu_length);
      if (bytes_read >0 )
	{
	  int pdu_num=-1;
	  if (bytes_read>3)
	    {
	      pdu_num = _buffer[0] <<24 | _buffer[1]<<16 |
		        _buffer[2] << 8 | _buffer[3];
	    }
	  cout << "Pdu["<< pdu_num <<"," <<_pdus<< "] "
	       <<  bytes_read << " bytes" << endl;
	  _total_bytes += bytes_read;
	  _pdus++;
	}
      else
	cout << "0 byte PDU!?" <<endl;
    }
    _data_callback++;
  }
  ...
  u_char _buffer[32768]; // see sean/xti/sean_io.h
  int _total_bytes;
  int _pdus;
  int _data_callback;
};
...
int main(int argc, char** argv) {
  if (argc!=2) {
    cout << "usage: " << argv[0] << " daemon_port\n";
    exit(1);
  }
  VisPipe("server5.output");

  DiagLevel("api",DIAG_ENV);
  
  int sd_port = atoi(argv[1]);
