...
"$Id: client5.cc,v 1.5 1998/09/28 12:31:44 bilal Exp $";
#endif
...
// not exit.
//
// NEW: When the call becomes active, it adds the second
...
private:
  ...
  Addr* _second_leaf;			// leaf we intend to add
  Outgoing_ATM_Call * _outcall;		// a call to _called_party

public:
  client_controller(Addr* who_to_call, Addr* who_to_add) : _outcall(0) { 
    _called_party = who_to_call;
    _second_leaf = who_to_add;
  }
...
private:
  ...
  void Call_Active(ATM_Call& call) {
    ...
    ATM_Leaf *leaf = Make_Leaf(call);

    // Supply the ATM address of the leaf
    ATM_Attributes::result err = leaf->Set_called_party_num(_second_leaf);

    assert(err == ATM_Attributes::success);

    cout << "Made leaf(id= " << leaf->Get_Leaf_ID() 
	 << ") to " << *_second_leaf << endl;
